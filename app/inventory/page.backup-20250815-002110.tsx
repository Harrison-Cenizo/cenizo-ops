...
export default function InventoryPage(){
  ...
  useEffect(()=>{
    ...
    setRunState(r); setRunSafe(r);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[runId, items.length, locKey]);

  function setRunSafe(r:Run){ setRunState(r); setRunSafe(r); }

  /** Counting controls */
  const cur = run?.lines[run.index];
  function nudge(d:number){ if(!run||!cur) return; const next = { ...run }; next.lines[run.index] = { ...cur, qty: Math.max(0, (cur.qty||0)+d) }; setRunSafe(next); }
  function setQty(q:number){ if(!run||!cur) return; const next = { ...run }; next.lines[run.index] = { ...cur, qty: Math.max(0, Math.floor(q)) }; setRunSafe(next); }
  function goNext(){ if(!run) return; setRunSafe({ ...run, index: Math.min(run.lines.length-1, run.index+1) }); }
  function goPrev(){ if(!run) return; setRunSafe({ ...run, index: Math.max(0, run.index-1) }); }
  function jumpTo(i:number){ if(!run) return; setRunSafe({ ...run, index:i }); }
  ...
  function applyPrefillToToday(){
    ...
    setRunSafe({ ...run, lines: nextLines });
              ...
              {(()=>{
                ...
                return (
                  <div className="flex flex-wrap gap-4">
                    <div><span className="text-gray-600">Supplier:</span> <span className="font-medium">{it.supplier||"Ã¢â‚¬â€"}</span></div>
                    <div><span className="text-gray-600">SKU:</span> <span className="font-medium">{it.sku||"Ã¢â‚¬â€"}</span></div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-auto" onClick={()=>setShowManage(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[1200px] p-4" onClick={e=>e.stopPropagation()}>
            ...
            <div className="flex gap-2 mb-3">
              <input className="border rounded p-2 flex-1" placeholder="SearchÃ¢â‚¬Â¦" value={filter} onChange={e=>setFilter(e.target.value)} />
      ...
      {showImport && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-auto" onClick={()=>setShowImport(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[1100px] p-4" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <div className="text-lg font-semibold">Import Toast CSV Ã‚Â· Build BOMs Ã‚Â· Prefill</div>
            ...
            {Object.keys(soldTotals).length>0 && (
              <div className="text-sm text-gray-700 mb-3">
                Distinct sold items: {Object.keys(soldTotals).length} Ã‚Â· Total units: {Object.values(soldTotals).reduce((a,b)=>a+b.qty,0)}
              </div>
            )}
            ...
            {Object.keys(soldTotals).length>0 && (
              <div className="space-y-3">
                ...
                {Object.values(soldTotals).filter(r=>!getBoms()[r.key]).length>8 && (
                  <div className="text-sm text-gray-600">Ã¢â‚¬Â¦save these and more will appear.</div>
                )}
              </div>
            )}
            ...
            {Object.keys(soldTotals).length>0 && (
              <div className="mt-4 flex gap-2">
                <button className="px-3 py-2 rounded border" onClick={applyPrefillToToday}>Apply Prefill to TodayÃ¢â‚¬â„¢s Run</button>
              </div>
            )}
          </div>
        </div>
