              ...
              {(()=>{
                ...
                return (
                  <div className="flex flex-wrap gap-4">
                    <div><span className="text-gray-600">Supplier:</span> <span className="font-medium">{it.supplier||"â€”"}</span></div>
                    <div><span className="text-gray-600">SKU:</span> <span className="font-medium">{it.sku||"â€”"}</span></div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-auto" onClick={()=>setShowManage(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[1200px] p-4" onClick={e=>e.stopPropagation()}>
            ...
            <div className="flex gap-2 mb-3">
              <input className="border rounded p-2 flex-1" placeholder="Searchâ€¦" value={filter} onChange={e=>setFilter(e.target.value)} />
      ...
      {showImport && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-auto" onClick={()=>setShowImport(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[1100px] p-4" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <div className="text-lg font-semibold">Import Toast CSV Â· Build BOMs Â· Prefill</div>
            ...
            {Object.keys(soldTotals).length>0 && (
              <div className="text-sm text-gray-700 mb-3">
                Distinct sold items: {Object.keys(soldTotals).length} Â· Total units: {Object.values(soldTotals).reduce((a,b)=>a+b.qty,0)}
              </div>
            )}
            ...
            {Object.keys(soldTotals).length>0 && (
              <div className="space-y-3">
                ...
                {Object.values(soldTotals).filter(r=>!getBoms()[r.key]).length>8 && (
                  <div className="text-sm text-gray-600">â€¦save these and more will appear.</div>
                )}
              </div>
            )}
            ...
            {Object.keys(soldTotals).length>0 && (
              <div className="mt-4 flex gap-2">
                <button className="px-3 py-2 rounded border" onClick={applyPrefillToToday}>Apply Prefill to Todayâ€™s Run</button>
              </div>
            )}
          </div>
        </div>
